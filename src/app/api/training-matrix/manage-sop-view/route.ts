...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    // Build sopCode → primaryDepartment map. Priority: MasterSOPRepository > SOPLibrary.
    const primaryDeptMap = new Map<string, string>();
    for (const doc of masterDocs as any[]) {
      if (!doc?.sopIdentifier) continue;
      const base = stripVersion(String(doc.sopIdentifier)).toUpperCase();
      if (!base || primaryDeptMap.has(base)) continue;
      const dept = normalizeDeptValue(doc.department);
      if (dept) primaryDeptMap.set(base, dept);
    }
    for (const doc of libraryDocs as any[]) {
      if (!doc?.sopIdentifier) continue;
      const base = stripVersion(String(doc.sopIdentifier)).toUpperCase();
      if (!base || primaryDeptMap.has(base)) continue;
      const dept = normalizeDeptValue((doc as any).department);
      if (dept) primaryDeptMap.set(base, dept);
    }
    ...
    // 2. MasterSOPRepository
    for (const doc of masterDocs as any[]) {
      recordName(doc.sopIdentifier, doc.language, doc.sopName);
    }
    // 3. SOPLibrary
    for (const doc of libraryDocs as any[]) {
      recordName(doc.sopIdentifier, doc.language, doc.sopName);
      if (doc.gujaratiName) recordGujarati(doc.sopIdentifier, doc.gujaratiName);
    }
    // 4. SOP collection — last resort
    ...
    // Add SOPs from MasterSOPRepository / SOPLibrary — these are the DB SOPs that may
    // have no training records, no assignments, AND no snapshot entry (the "unassigned"
    // bucket the main training-matrix page counts as missing-from-Excel).
    for (const doc of masterDocs as any[]) {
      if (!doc?.sopIdentifier) continue;
      const base = stripVersion(String(doc.sopIdentifier)).toUpperCase();
      if (!base || sopSet.has(base)) continue;
      const sopName = resolveName(base, doc.sopName);
      if (isValidSopCode(base) && isValidSopName(sopName)) {
        sopSet.set(base, sopName);
      }
    }
